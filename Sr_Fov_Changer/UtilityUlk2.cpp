        ...
        if (instanceId == g_currentTestInstanceId) {
            ...
            if (_oldvalue == 30.0f || _foundsrfovtarget == true)
            {
                if (!_foundsrfovtarget)
                {
                    ...
                    //DebugPrint("[DLL] SUCCESS: Found target FOV function at instance %d! Value: %.4f, Address: 0x%p\n",
                    //    instanceId, _oldvalue, (void*)instanceAddress);
...
bool InstallSRFOVHook(int maxHookCount = 600,int SrFovHookSpeed = 150,int SrFovHookRtime = 1000) {
    ...
    std::vector<uintptr_t> setFieldOfViewAddrs = PatternScanner::MultipleScan(_srfovpattern.c_str());
