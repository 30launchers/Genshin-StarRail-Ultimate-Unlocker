...
namespace UnlockFps.Gui.Views
{
    public partial class InitializationWindow : Window
    {
        ...
        private bool foundGameSection = false;
        ...
        private void StartSearchWindow()
        {
            Task.Run(async () =>
            {
                ...
                await Dispatcher.UIThread.InvokeAsync(async () =>
                {
                    ...
                    if(foundGameSection == true)
                    {
                        infoWindow.Text = $"""
                                           Game Found!
                                           {_configService.Config.LaunchOptions.SRGamePath}
                                           """;
                    }
                    else
                    {
        ...
        private async ValueTask<bool> FindWindowAsync()
        {
            ...
            // 获取文件名并判断游戏类型
            string fileName = Path.GetFileName(processPath).ToLower();

            if (fileName == "starrail.exe")
            {
                _configService.Config.LaunchOptions.SRGamePath = Path.GetFullPath(processPath);
                foundGameSection = true;
            }
            else if (fileName == "yuanshen.exe" || fileName == "genshinimpact.exe")
            {
                _configService.Config.LaunchOptions.GamePath = Path.GetFullPath(processPath);
                foundGameSection = false;
            }
            else
            {
                // 处理未知的可执行文件
                var alertWindow = App.DefaultServices.GetRequiredService<AlertWindow>();
                alertWindow.Text = $"Unknown game executable: {fileName}\nPlease use \"Browse\" instead.";
                await alertWindow.ShowDialog(this);
                return false;
            }


            //if (_configService.Config.GameSelection == false) 
            //{
            //    _configService.Config.LaunchOptions.GamePath = Path.GetFullPath(processPath);
            //}

            //if (_configService.Config.GameSelection == true)
            //{
            //    _configService.Config.LaunchOptions.SRGamePath = Path.GetFullPath(processPath);
            //}
...
#pragma warning restore CA1416
        ...
        private async void BtnBrowse_OnClick(object? sender, RoutedEventArgs e)
        {
            ...
            var directory = Path.GetDirectoryName(selectedPath);
            var mhypbaseFile = Path.Combine(directory, "mhypbase.dll");
            if (!File.Exists(mhypbaseFile))
            {
                var alertWindow = App.DefaultServices.GetRequiredService<AlertWindow>();
                alertWindow.Text = "That's not the right place";
                await alertWindow.ShowDialog(this);
        ...
        private async void BtnBrowseSR_OnClick(object? sender, RoutedEventArgs e)
        {
            var selectedPath = (await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
            {
                Title = "Select StarRail.exe",
                FileTypeFilter =
                [
                    new FilePickerFileType("Executable Files (*.exe)") { Patterns = ["StarRail.exe"]}
                ],
                AllowMultiple = true
            })).FirstOrDefault()?.TryGetLocalPath();
            ...
            _configService.Config.LaunchOptions.SRGamePath = selectedPath;
            _configService.Save();
            Close();
        }
    }
